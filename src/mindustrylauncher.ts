...
export async function copyMods(state:State){
	const modTasks = state.externalMods.map(async mod => {
		if(mod.type == "java"){
			...
			const modFileName = (await fsP.readdir(modBuildFolder).catch(async ():Promise<never> => {
				//mod build folder doesn't exist
				//Check for build.gradle
				if(!state.buildMods) {
					const gradleExists = fsP.access(path.join(mod.path, "build.gradle"), fs.constants.R_OK);
					gradleExists.catch(() => {});
					await gradleExists.then(() =>
						fail(`Could not read the build folder at ${mod.path}. Please build the mod first by passing "--build".`)
					);
				}
...
export class Version {
	...
	static async fromInput(version:string, state:State){
		...
		if(state.settings.mindustryJars.customVersionNames[version]){
			...
			if(!fs.existsSync(filepath)) fail(`Invalid custom version ${version}: specified filepath ${filepath} does not exist.`);
			...
			let cleanVersion;
			if(version.startsWith("v") && version.length > 1) cleanVersion = version.slice(1);
			else cleanVersion = version;
	...
	async getDownloadUrl(){
		...
		return {
			url: await resolveRedirect(versionData.url(this.versionNumber)).catch(err => {
				if(
					err instanceof Error && err.message == "Version does not exist." &&
					this.versionType?.startsWith("foo")
				){
					const possibleVersions = (["foo-v6", "foo", "foo-v8"] satisfies Array<keyof typeof versionUrls>).filter(v => v !== this.versionType);
